...
```{r}
files <- list.files(path="~/Desktop/git MP/DESeq analysis/raw counts", pattern="*.txt", full.names=TRUE, recursive=FALSE)
...
ggsave("~/Desktop/git MP/rlogPCA.pdf", device="pdf", width = 7, height = 5) # to save the plot
```
...
cdplot1
ggsave("~/Desktop/git MP/volcanoMP.pdf", device="pdf", width = 8, height = 5)
...
```{r}
conversion<- read.table("~/Desktop/git MP/Genome resources/gene_correspondence_table_v51.txt", fill = TRUE, row.names=NULL, col.names = c("V1", "old_ID"))
cd_table_conv<-cd_tableMP %>%
    left_join(conversion, by=c("ensgenes"="V1"))

#add the annotation
annot<- read.csv("~/Desktop/git MP/Genome resources/MpTak_v6.1_func_annot_1line_excel.csv", sep= ";", header=FALSE)
...
Add GO terms
```{r}
GOlist<- read.csv("~/Desktop/git MP/Genome resources/MpTak1v5.1r1_GO_annotation.tsv", sep="", header = FALSE)
...
CdDownMP<- cd_full_GO[cd_annot$padj <= 0.05 & cd_full_GO$log2FoldChange <= -1, ]
...
cd_ordered_GO<- cd_frame_na[,c(2,3,1)]
...
```{r}
annotKEGG<- read.csv("~/Desktop/git MP/Genome resources/MpTak_v6.1_func_annotation.tsv", sep= "", header=FALSE)
...
MPpaths_sig<- MPpaths %>%
  filter(P.DE<0.05)
write.csv(MPpaths_sig, "~/Desktop/git MP/kegga_sig.csv")
...
write.csv(Kegg_clean, "~/Desktop/git MP/keggtrial.csv")
